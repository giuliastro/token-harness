...
describe('update', () => {
  ...
  it('checks and updates Token Harness itself from its verified global npm installation', async () => {
    ...
    assert.equal(preview.data?.application?.installed, '0.1.20');
    assert.equal(preview.data?.application?.available, '0.1.21');
    ...
    assert.equal(applied.data?.application?.installed, '0.1.21');
    ...
    assert.ok(applied.asked.includes('npm install --global token-harness@0.1.21'));
    assert.equal(
  ...
  it('does not update a source or temporary copy just because a global npm package exists', async () => {
    ...
    writeFileSync(
      ...
      JSON.stringify({ name: 'token-harness', version: '0.1.21' }),
    );
    writeFileSync(
