...
- `observed` when an existing passive canary proves runtime use **for that exact harness**;
- `not-exercised` when the integration is configured but its passive canary has not produced a qualifying receipt yet;
- `failed` when an actual passive verification check fails;
- `unavailable` when no passive runtime witness can be attributed to that exact pair.
...
Passive evidence is harness-scoped before it reaches this capture. HarnessTrim can do that directly because its native `TrimEvent` carries a `harness` identity. Provider-wide telemetry without a harness identity may be attributed by exclusion only when exactly one harness is wired. If multiple harnesses share that provider, the runtime receipt remains `unavailable` for each exact pair instead of being copied across them. A verification row can therefore be below a declared runtime tier because attribution is unavailable without implying that the integration itself failed.
...
- an actual failed passive check should be fixed before review;
...
- an `unavailable` witness — including provider-wide telemetry that cannot identify which of multiple wired harnesses produced it — likewise requires controlled benchmark or manual evidence.
