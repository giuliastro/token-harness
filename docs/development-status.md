...
- 2026-09-14: candidate lifecycle success now carries an explicit generic `requestedStateVerified` result bit. The simple CLI does not send users to a redundant `token-harness verify` after a candidate apply/deactivation that already verified its requested state; ordinary managed apply still recommends `verify` when that independent check remains useful.
