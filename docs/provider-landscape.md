...
| [Claude Code Router](https://github.com/musistudio/claude-code-router) | MIT | Agent-native model gateway for coding agents | One local endpoint for Claude Code, Codex, OpenCode, and other agents, with request-conditioned routing rules and request logs. RFC 0028 selects it for a local heuristic shadow experiment across Claude Code and Codex. | The first slice exports a CCR Node.js script rule and does not install CCR, alter its config, or enable a paid provider. Any later managed path still needs endpoint/profile ownership, credential redaction, verification, rollback, and attributable per-request quality/usage evidence. Routing outcomes stay out of token-saving totals. |
...
The CCR shadow script is not provider admission or proof of a verified route. Any future managed
CCR lifecycle or alternative router remains gated on the list above and the measured policy gates in
RFC 0028. Native model/effort/context policy stays first.
