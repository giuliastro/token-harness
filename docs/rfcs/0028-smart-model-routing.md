...
gateway, replace native harness policy, install CCR, or silently change user-owned harness
configuration.
...
request/response bodies. Telemetry-write failure does not block the CCR request. Users can inspect
the event counts and explicitly prune old local records.
...
offline tests. The user adds the exported Node.js script through CCR's Routing UI. Token Harness does
not install CCR, create CCR provider credentials, alter CCR rules, or claim live-gateway verification
in this slice.
