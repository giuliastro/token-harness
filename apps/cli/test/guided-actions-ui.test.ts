...
describe('guided actions UI', () => {
  ...
  it('explains optimizer setup before running a preview', () => {
    ...
    assert.match(GUIDE_JS, /The Apply button appears only if Token Harness can build a safe, concrete change plan/);
  });
  ...
  it('keeps setup routed through the existing preview and apply transaction endpoints', () => {
    assert.match(GUIDE_JS, /post\('\/api\/preview', \{ action: 'setup', harness: agent \}\)/);
    assert.match(GUIDE_JS, /post\('\/api\/apply', \{ ticket \}\)/);
    assert.match(GUIDE_JS, /Backups, ownership checks and verification run as part of the transaction/);
