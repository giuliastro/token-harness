...
export interface DashboardHarness {
  ...
  allowanceIssue: { code: string; message: string; command: string | null } | null;
}
...
function allowanceIssue(
  budget: BudgetReport['harnesses'][number] | undefined,
): DashboardHarness['allowanceIssue'] {
  if (budget !== undefined && budget.windows.length > 0) return null;
  const known: Readonly<Record<string, string>> = {
    'cclimits-not-installed':
      'Claude quota needs the optional cclimits companion, which was not found in this terminal.',
    'cclimits-readonly-flags-unsupported':
      'The installed cclimits build is too old for read-only Claude quota. Check its release and supported flags.',
    'cclimits-claude-source-unsupported':
      'The installed cclimits response does not include supported Claude discovery metadata. Check its release.',
    'cclimits-python-unavailable': 'cclimits was found, but its Python runtime could not start.',
    'cclimits-claude-credentials-unavailable':
      'cclimits found no existing Claude login or usable usage snapshot. Check its version and the Claude login.',
    'cclimits-claude-token-expired':
      'The existing Claude login has expired. Check the login in Claude and retry.',
    'cclimits-claude-cache-stale':
      'Only stale Claude usage data was available. It is not shown as current allowance.',
    'cclimits-claude-schema-unrecognized':
      'cclimits returned an unsupported quota format. Check the installed build.',
    'cclimits-claude-usage-unavailable':
      'cclimits could not read Claude quota. The optimizer can still be used normally.',
  };
  const problem = budget?.diagnostics.find((item) => Object.hasOwn(known, item.code));
  return {
    code: problem?.code ?? 'allowance-unavailable',
    message:
      problem === undefined
        ? 'Current allowance could not be observed. Use the agent native usage view.'
        : (known[problem.code] as string),
    command:
      budget?.harnessId === 'claude'
        ? 'token-harness budget --harness claude --verbose'
        : budget?.harnessId === 'codex'
          ? 'token-harness budget --harness codex --verbose'
          : null,
  };
}
...
export function buildDashboardModel(input: {
  ...
  // Claude and Codex are primary product targets even before an optimizer or quota companion
  // is configured. Otherwise a missing companion hides the very card that explains its absence.
  for (const item of present) {
    if (item.harnessId === 'claude' || item.harnessId === 'codex')
      relevantHarnessIds.add(item.harnessId);
  }
  ...
  const harnesses: DashboardHarness[] = relevantHarnesses.map((harness) => {
    ...
    return {
      ...
      allowanceIssue: allowanceIssue(budget),
      allowance: (budget?.windows ?? []).map((window) => ({
        label: [window.bucketName, window.scope === 'five-hour' ? '5-hour' : window.scope]
          .filter(Boolean)
          .join(' / '),
